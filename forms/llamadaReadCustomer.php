...
if(!empty($_GET['usuario'])){
    ...
    $url_dame_clientes .= "usuario=$usuario&";
}
if(!empty($_GET['nombre'])){
    $nombre=$_GET['nombre'];
    $nombre=str_replace(" ", "+", $nombre);
    $url_dame_clientes .= "nombre=$nombre&";
}
...
if($xml->status == 'OK'){
    ...
    $output = "<h1>CLIENTES</h1><ul class=\"busqueda\">";
    $i=0;
    foreach ($xml->cliente as $cliente) {
        $i++;
        $output .= "<li onclick=\"copiarTexto('textoACopiar{$i}')\"><span class='icono'>🌟</span><span id=\"textoACopiar{$i}\">{$cliente->nombre} {$cliente->primerApellido} {$cliente->segundoApellido}</span>({$cliente->edad} años) ➪ {$cliente->usuario}</li>";
    }
