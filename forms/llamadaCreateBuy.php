...
<html>
    ...
    <div class="content">
        <h1><b>COMPRAS</b></h1>
<?php
// Ruta al archivo readCustomer.php en tu servidor
$fecha = $_GET['fecha'];
$idCliente = $_GET['idCliente'];
$url_create_buy="http://iescristobaldemonroy.duckdns.org:81/USER48/TiendaJuegosManagement/services/createBuy.php?fecha=$fecha&idCliente=$idCliente";
...
foreach ($xml->producto as $producto) {
    ...
    if(isset($_GET["ID".$idProducto])){
        $idCheck=$_GET["ID".$idProducto];
        if(!empty($idCheck)){
            $cantidad=$_GET["cantidad".$idProducto];
...
// Hacer la solicitud HTTP y obtener el XML como una cadena
$xmlString = file_get_contents($url_create_buy);
...
$output = $xml->status;
if($output=="OK"){
    echo "<h1>La compra se ha insertado correctamente.</h1>";
    $idCompra = $xml->idCompra;

    foreach ($productos as $clave => $valor) {
        $url_create_purchaseLine="http://iescristobaldemonroy.duckdns.org:81/USER48/TiendaJuegosManagement/services/createPurchaseLine.php?idCompra=$idCompra&idProducto=$clave&cantidad=$valor";
        
        // Hacer la solicitud HTTP y obtener el XML como una cadena
        $xmlString = file_get_contents($url_create_purchaseLine);
    ...
    if($output == "ERROR"){
        echo "<h1>No se ha podido insertar la compra.</h1>";
        echo "<h2>Vuelve a revisar los datos. Es posible que exista la compra con usuario $usuario.</h2>";
    }
}
?>
