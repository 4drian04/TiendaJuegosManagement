...
<html>
    ...
    <div class="content">
<?php
// Ruta al archivo readProduct.php en tu servidor
...
if(!empty($_GET['nombre'])){
    ...
    $url_dame_productos .= "nombre=$nombre&";
}
if(!empty($_GET['plataforma'])){
    $plataforma=$_GET['plataforma'];
    $plataforma=str_replace(" ", "+", $plataforma);
    $url_dame_productos .= "plataforma=$plataforma";
}
...
if($xml->status == 'OK'){
    ...
    $output = "<h1>PRODUCTOS</h1><ul class=\"busqueda\">";
    $i=0;
    foreach ($xml->producto as $producto) {
        $i++;
        $ternariaPlataforma = $producto->plataforma==""?"":"($producto->plataforma)";
        $output .= "<li onclick=\"copiarTexto('textoACopiar{$i}')\"><span class='icono'>🌟</span><span id=\"textoACopiar{$i}\">{$producto->nombre}</span>$ternariaPlataforma - {$producto->precio}€ ➪ {$producto->proveedor}</li>";
    }
