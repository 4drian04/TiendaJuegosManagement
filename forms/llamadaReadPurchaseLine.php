...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Líneas de compra ARVID</title>
    ...
    <div class="content">
        <h1><b>Linea de Compra</b></h1>
<?php
// Ruta al archivo readPurchaseLine.php en tu servidor
$url_dame_lineasCompra = "http://iescristobaldemonroy.duckdns.org:81/USER48/TiendaJuegosManagement/services/readPurchaseLine.php?";
if(!empty($_GET['nombreProducto'])){
    $nombreProducto = $_GET['nombreProducto'];
    $nombreProducto = str_replace(" ", "+", $nombreProducto);
    $url_dame_lineasCompra .= "nombreProducto=$nombreProducto&";
}
if(!empty($_GET['fecha'])){
    $fecha = $_GET['fecha'];
    $url_dame_lineasCompra .= "fecha=$fecha&";
}
if(!empty($_GET['nombreCliente'])){
    $nombreCliente = $_GET['nombreCliente'];
    $nombreCliente=str_replace(" ", "+", $nombreCliente);
    $url_dame_lineasCompra .= "nombreCliente=$nombreCliente&";
}
if(!empty($_GET['apellido1'])){
    $apellido1 = $_GET['apellido1'];
    $apellido1=str_replace(" ", "+", $apellido1);
    $url_dame_lineasCompra .= "apellido1=$apellido1&";
}
if(!empty($_GET['apellido2'])){
    $apellido2 = $_GET['apellido2'];
    $apellido2=str_replace(" ", "+", $apellido2);
    $url_dame_lineasCompra .= "apellido2=$apellido2";
}

// Hacer la solicitud HTTP y obtener el XML como una cadena
$xmlString = file_get_contents($url_dame_lineasCompra);

// Verificar si la solicitud fue exitosa
if ($xmlString === FALSE) {
    die('Error al obtener el XML de readPurchaseLine.php');
}
...
if($xml->status == 'OK'){
    ...
    foreach ($xml->linea_compra as $linea_compra) {
        $i++;
        $precioTotal=$linea_compra->cantidad*$linea_compra->precio;
        $output .= "<li onclick=\"copiarTexto('textoACopiar{$i}')\"><span class='icono'>🌟</span>{$linea_compra->nombreCliente} {$linea_compra->apellido1} {$linea_compra->apellido2} ha comprado {$linea_compra->cantidad} <span id=\"textoACopiar{$i}\">{$linea_compra->nombreProducto}</span> ({$precioTotal}€)</li>";
    }
