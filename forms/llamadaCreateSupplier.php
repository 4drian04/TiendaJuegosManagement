...
<?php
// Ruta al archivo createSupplier.php en tu servidor
$nombre = $_GET['nombre'];
$nombre=str_replace(" ", "+", $nombre);
$url_insertar_proveedor="http://iescristobaldemonroy.duckdns.org:81/USER48/TiendaJuegosManagement/services/createSupplier.php?nombre=$nombre";
if(!empty($_GET['telefono'])){
    $telefono = $_GET['telefono'];
    $telefono=str_replace(" ", "+", $telefono);
    $url_insertar_proveedor .="&telefono=$telefono";
}


// Hacer la solicitud HTTP y obtener el XML como una cadena
$xmlString = file_get_contents($url_insertar_proveedor);

// Verificar si la solicitud fue exitosa
if ($xmlString === FALSE) {
    die('Error al obtener el XML de createSupplier.php');
}
...
$output = $xml->status;
if($output=="OK"){
    echo "<h1>El proveedor se ha insertado correctamente.</h1>";
}else{
    if($output == "ERROR"){
        echo "<h1>No se ha podido insertar el proveedor.</h1>";
    }
}
?>
