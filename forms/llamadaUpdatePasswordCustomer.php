...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Cambiar contraseña de clientes de ARVID</title>
    ...
    <div class="content">
        <h1>CAMBIAR CONTRASEÑA DE CLIENTE</h1>
<?php
// Ruta al archivo updatePasswordCustomer.php en tu servidor
$usuario = $_GET['usuario'];
$usuario=str_replace(" ", "+", $usuario);
$contrasenhaAntigua = $_GET['contrasenhaAntigua'];
$contrasenhaAntigua=str_replace(" ", "+", $contrasenhaAntigua);
$contrasenhaNueva = $_GET['contrasenhaNueva'];
$contrasenhaNueva=str_replace(" ", "+", $contrasenhaNueva);
$url_cambiar_contrasenha="http://iescristobaldemonroy.duckdns.org:81/USER48/TiendaJuegosManagement/services/updatePasswordCustomer.php?&usuario=$usuario&contrasenhaAntigua=$contrasenhaAntigua&contrasenhaNueva=$contrasenhaNueva";


// Hacer la solicitud HTTP y obtener el XML como una cadena
$xmlString = file_get_contents($url_cambiar_contrasenha);

// Verificar si la solicitud fue exitosa
if ($xmlString === FALSE) {
    die('Error al obtener el XML de updatePasswordCustomer.php');
}
...
$output = $xml->status;
if($output=="OK"){
    echo "<h1>La contraseña se ha cambiado correctamente.</h1>";
}else{
    if($output == "ERROR"){
        echo "<h1>No se ha podido cambiar la contraseña.</h1>";
        echo "<h2>Vuelve a revisar los datos. Es posible que no exista el cliente con usuario $usuario o la contraseña es incorrecta.</h2>";
    }
}
?>
