...
<html>
    ...
    <div class="content">
        <h1><b>PRODUCTOS</b></h1>
<?php
// Ruta al archivo createProduct.php en tu servidor
$nombre = $_GET['nombre'];
$nombre=str_replace(" ", "+", $nombre);
$precio = $_GET['precio'];
$url_insertar_productos="http://iescristobaldemonroy.duckdns.org:81/USER48/TiendaJuegosManagement/services/createProduct.php?nombre=$nombre&precio=$precio";
if(!empty($_GET['plataforma'])){
    $plataforma = $_GET['plataforma'];
    $plataforma=str_replace(" ", "+", $plataforma);
    $url_insertar_productos .="&plataforma=$plataforma";
}
if(!empty($_GET['idProveedor'])){
    $idProveedor = $_GET['idProveedor'];
    $url_insertar_productos .="&idProveedor=$idProveedor";
}


// Hacer la solicitud HTTP y obtener el XML como una cadena
$xmlString = file_get_contents($url_insertar_productos);
...
$output = $xml->status;
if($output=="OK"){
    echo "<h1>El producto se ha insertado correctamente.</h1>";
}else{
    if($output == "ERROR"){
        echo "<h1>No se ha podido insertar el producto.</h1>";
    }
}
?>
