...
try{
    ...
    //Escribimos la sentencia para poder eliminar un proveedor
    $sql = $con->prepare('DELETE FROM proveedor WHERE nombre=?');
    ...
    if($sql->execute()){ //Si la operación se ha hecho correctamente, le indicamos que la operación se ha hecho de manera exitosa
        $responseXML = Utilities::generateResponseXML('OK', 'Proveedor eliminado con éxito.');

        header('Content-Type: application/xml');
        echo $responseXML->asXML();
    }else{ //En caso contrario, le indicamos que no se ha podido eliminar el proveedor
        throw new Exception("Error al eliminar el proveedor: " . $sql->error);
    }
